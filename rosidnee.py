...
def menu():
	...
	print(" [*] ---------------------------------------") 
	print(" [+] Author : https://www.facebook.com/profile.php?id=100018760997097&mibextid=ZbWKwL") 
	print(" [+] GitHub : https://github.com/Rosid456")
	print(" [*] ---------------------------------------") 
	print(" [1]. crack dari random new")
	print(" [2]. crack dari random old")
	print(" [3]. crack dari nama email")
	print(" [4]. lihat akun ROSID crack")
	ask = input("\n [?] pilih : ")
	if ask in["1"]:
		random_new()
	elif ask in["2"]:
		...
		print("\n [1] lihat akun ROSID crack ok")
		print(" [2] lihat akun ROSID crack cp")
		ROSID = input("\n [?] pilih : ")
		if ROSID in["1"]:
			try:ROSIDok = open("ok.txt").read().splitlines()
			except:exit(f" [!] tidak ada file ok, silahkan crack dulu")
			print(" [#] ----------------------------------------------")
			print(f" [+] total akun ROSID crack ok : {len(ROSIDok)}")
			print(f" [#] ----------------------------------------------{H}")
			os.system("cat ok.txt")
			exit(f"\n\n {N}[#] selesai cek ...")
		elif ROSID in["2"]:
			try:ROSIDcp = open("ok.txt").read().splitlines()
			except:exit(f" [!] tidak ada file cp, silahkan crack dulu")
			print(" [#] ----------------------------------------------")
			print(f" [+] total akun ROSID crack ok : {len(ROSIDcp)}")
			print(f" [#] ----------------------------------------------{K}")
...
def atursandi():
	...
	print("\n [+] ROSID crack ok tersimpan di -> ok.txt")
	print(" [+] ROSID crack cp tersimpan di -> cp.txt\n")
